...
let diff name program input expected =
  let exps =
    try Ok (S_exp.parse_many program |> Lisp_syntax.program_of_s_exps) with e -> Error (Printexc.to_string e)
  in
  let try_bind f arg =
...
let starts_with prefix s =
  try (String.compare (String.sub s 0 (String.length prefix)) prefix == 0) with
...
let csv_results =
  ...
  |> List.mapi (fun i ->
         let anon_name = sprintf "anonymous-%d" i in
         function
         | [name; program] when starts_with "name:" name ->
             [(name, diff name program "" None)]
         | [name; program; (("error" | "ERROR") as error)] when starts_with "name:" name ->
             [(name, diff name program "" (Some (Error error)))]
         | [name; program; expected] when starts_with "name:" name ->
             [(name, diff name program "" (Some (Ok expected)))]
         | [name; program; input; (("error" | "ERROR") as error)] when starts_with "name:" name ->
             [(name, diff name program input (Some (Error error)))]
         | [name; program; input; expected] when starts_with "name:" name ->
             [(name, diff name program input (Some (Ok expected)))]
         | name :: program :: pairs when starts_with "name:" name ->
         ...
         | [program] ->
             [(anon_name, diff anon_name program "" None)]
         | [program; (("error" | "ERROR") as error)] ->
             [(anon_name, diff anon_name program "" (Some (Error error)))]
         | [program; expected] ->
             [(anon_name, diff anon_name program "" (Some (Ok expected)))]
         | [program; input; (("error" | "ERROR") as error)] ->
             [(anon_name, diff anon_name program input (Some (Error error)))]
         | [program; input; expected] ->
             [(anon_name, diff anon_name program input (Some (Ok expected)))]
         | program :: pairs ->
             let rec diff_multiple i = function
               | [] ->
                   []
               | input :: expected :: rest ->
                   let name = sprintf "%s-%d" anon_name i
                   and expected =
                     match expected with
                     | ("error" | "ERROR") as error ->
                         Error error
